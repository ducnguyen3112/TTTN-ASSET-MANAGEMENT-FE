...
const EditUserMain = (props) => {
    ...
    const [joinedDateChangeMessageError, setJoindedDateMessageError] = useState();
    ...
    useEffect(() => {
        ...
        const getRoles = async () => {
            try {
                const getRoleFromDatabase = await axios
                    .get(`${AXIOS_API_URL}/admin/api/roles`, {
                        headers: { Authorization: 'Bearer ' + window.localStorage.getItem('accessToken') },
                    })
    ...
    useEffect(() => {
        ...
        axios
            .put(
                `${AXIOS_API_URL}/admin/api/users/checkassignment/${staffCode}`,
                {
                    firstName: firstName,
                    lastName: lastName,
                    birthDate: birthDate,
            ...
            .then((response) => {
                ...
                setJoindedDateMessageError(response.data.message);
            })
            .catch((error) => {
    ...
    const handleEditUser = () => {
        ...
        axios
            .put(
                `${AXIOS_API_URL}/admin/api/users/${staffCode}`,
                {
                    firstName: firstName,
                    lastName: lastName,
                    birthDate: birthDate,
            ...
            .then((res) => {
                console.log('handleEditUser :' + res.data.staffCode);
                console.log('handleEditUser :' + res.data.joinedDate);
                ...
                <FormContent>
                    ...
                    <FormContentItem>
                        <InputDate
                            ...
                            className={
                                joinedDateSaturdaySundayError || joinedDateSoonError ? 'borderDanger' : 'borderPrimary'
                            }
                        ></InputDate>
                        {joinedDateChangeMessageError && (
                            <Error style={{ color: 'yellow' }}>{joinedDateChangeMessageError}</Error>
                        )}
                        {joinedDateSaturdaySundayError && (
                            <Error>
                                Joined date is Saturday or Sunday.
                                <br /> Please select a different date
                            </Error>
                        )}
                        {joinedDateSoonError && (
                            <Error>
                                Joined date is not later than Date of Birth.
                                <br /> Please select a different date
                            </Error>
                        )}
                    </FormContentItem>
                                      ...
                                      if (role.name === props.data.roleName) {
                                          return (
                                              <Option value={role.name} selected>
                                                  {StringFormatter.capitalizeFirstLetter(role.name)}
                                              </Option>
                                          );
                                      }
                                      return (
                    ...
                    <Button>
                        ...
                        <ButtonContainer>
                            <ButtonClick id="editUser_Cancel" onClick={() => navigate('/manage-user')}>
                                Cancel
                            </ButtonClick>
                        </ButtonContainer>
                    </Button>
